...
///
/// </summary>
public class OVRCardboardController : MonoBehaviour
{
	...
	{
		// Get the cameras
		OVRCardboard[] cameras = gameObject.GetComponentsInChildren<OVRCardboard>();
	...
	void Start()
	{
		...
		{
			OVRCardboard[] ovrCameras = gameObject.GetComponentsInChildren<OVRCardboard>();
	...
	{
		OVRCardboard cam = camera.GetComponent<OVRCardboard>();
	...
	public void InitCameraControllerVariables()
	{
		...
		VerticalFOV = CameraMain.GetComponent<OVRCardboard>().GetIdealVFOV();
		// Get aspect ratio as well
		AspectRatio = CameraMain.GetComponent<OVRCardboard>().CalculateAspectRatio();
